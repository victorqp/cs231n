...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dprev_h += dh[:, i, :]
        dxi, dprev_h, dWxi, dWhi, dbi = rnn_step_backward(dprev_h, cache[i])
        dx[:, i, :] = dxi
        dWx += dWxi
        dWh += dWhi
        db += dbi
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    g = np.tanh(block_input)
    next_c = f * prev_c + i * g
    next_h = o * np.tanh(next_c)
    cache = (i, f, o, g, next_c, prev_c, Wx, Wh, prev_h, x)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    i, f, o, g, next_c, prev_c, Wx, Wh, prev_h, x = cache
    dtanh_next_c = dnext_h * o
    dnext_c2 = dtanh_next_c * (np.ones_like(next_c) - np.tanh(next_c) ** 2)
    dprev_c = (dnext_c + dnext_c2) * f
    di = (dnext_c + dnext_c2) * g
    df = (dnext_c + dnext_c2) * prev_c
    do = dnext_h * np.tanh(next_c)
    dg = (dnext_c + dnext_c2) * i
    dig = di * i * (np.ones_like(i) - i)
    dfg = df * f * (np.ones_like(f) - f)
    dog = do * o * (np.ones_like(o) - o)
    dbi = dg * (np.ones_like(g) - g ** 2)
    da = np.hstack((dig, dfg, dog, dbi))
    db = np.sum(da, axis=0)
    dx = da.dot(Wx.T)
    dWx = x.T.dot(da)
    dWh = prev_h.T.dot(da)
