...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        #loss and grads of the loss layer
        loss, dloss = softmax_loss(scores, y)
        W1, W2 = self.params['W1'], self.params['W2']
        loss += 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
        #grads of the output layer
        dout, grads['W2'], grads['b2'] = affine_backward(dloss, out_cache)
        #add grad from regularization
        grads['W2'] += self.reg * W2
        #grads of the affine-relu layer
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dout, h1_cache)
        #add grad from regularization 
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            w_name = 'W' + str(i+1)
            b_name = 'b' + str(i+1)
            self.params[w_name] = np.random.randn(dims[i], dims[i+1]) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        # list to store cache from each hidden layer
        hidden_cache = []
        dropout_cache = []
        # initialize the first output as input data X
        out = X
        ...
        # initialize the derivative from last layer to loss derivative
        d_last = dloss
        d_last, dw, db = affine_backward(d_last, hidden_cache[self.num_layers-1])
        grads['W'+str(self.num_layers)] = dw + self.reg * self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = db
        for i in reversed(range(self.num_layers-1)):
            if self.use_dropout:
                d_last = dropout_backward(d_last, dropout_cache[i])
            if self.use_batchnorm:
                d_last, dw, db, dgamma, dbeta = affine_bn_relu_backward(d_last, hidden_cache[i])
                grads['W'+str(i+1)] = dw + self.reg * self.params['W'+str(i+1)]
                grads['b'+str(i+1)] = db
                grads['gamma'+str(i+1)] = dgamma
                grads['beta'+str(i+1)] = dbeta
            else:
                d_last, dw, db = affine_relu_backward(d_last, hidden_cache[i])
                grads['W'+str(i+1)] = dw + self.reg * self.params['W'+str(i+1)]
                grads['b'+str(i+1)] = db
